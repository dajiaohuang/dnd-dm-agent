...
运行过程中，通过自然语言生成的代码必须经过函数化改造后记录到 `code/` 代码库中，以便下次直接调用。
...
3. **分类存放**：放入 `code/<category>/<filename>.py`
...
| 分类 | 目录 | 内容 |
|------|------|------|
| 🎲 骰子 | `code/dice/` | 投骰、随机数、属性生成 |
| ⚔️ 战斗 | `code/combat/` | 先攻表、命中判断、伤害结算 |
| 💾 存档 | `code/save/` | 存档读写、自动编号 |
| 📦 模组 | `code/module/` | 模组扫描、章节解析、结构构建 |
| 👥 角色 | `code/party/` | 角色状态、经验计算、物品检索 |
...
`DM_TEMPLATES.md` 中定义的输出模板（如战斗表格式、法术展示表、角色卡布局等）与 `code/` 中对应的生成函数**必须保持同步**。模板改 → 代码改，反之亦然。
...
| DM_TEMPLATES.md 9.3 战斗态势表 | `code/combat/display.py:build_combat_table()` | 列顺序/列名/格式必须一致 |
| DM_TEMPLATES.md 9.4 地形图 | `code/combat/formulas.py:environment_text()` | 地形/光线/特殊字段顺序一致 |
| DM_TEMPLATES.md 9.5 任务清单 | `code/save/templates.py:new_quest_template()` | 任务状态字段格式一致 |
| DM_TEMPLATES.md 9.6 角色卡 | `code/party/live.py:get_all_characters()` | 属性顺序/装备展示方式一致 |
| DM_TEMPLATES.md 9.7 队伍总览 | `code/party/live.py:get_party_summary()` | 摘要字段一致 |
| DM_TEMPLATES.md 规则11 法术展示 | 无独立函数（由LLM查模板直接生成） | 模板改后LLM输出自动同步 |

### 5.3 触发同步的变更类型

以下变更要求同时更新 `DM_TEMPLATES.md` 和 `code/`：
...
需要修改模板 → 在 DM_TEMPLATES.md 中修改
    ↓
查找 code/CODE_LIBRARY.md 中对应的生成函数
    ↓
同步修改函数参数/返回格式/输出文本
    ↓
验证：运行函数查看输出是否与模板一致
...
> **在 code/ 代码库新增任何函数前，必须优先检索 srd/scripts/ 中的 Python 脚本是否能满足需求。SRD 已有功能的，强制引用 SRD 代码，禁止重复造轮。**

安装 dnd5e-srd skill 后，工作目录 srd/ 提供了完整的 SRD 5.2.1 RAG 检索能力（search_with_positions.py + expand_context.py），覆盖法术数据、职业特性、装备参数、怪物数据、状态定义等大量结构化数据。在 code/ 中新增函数时，如所需数据 SRD 已有，优先通过搜索脚本获取，而非自建数据库或硬编码数据。
...
第一步：搜索 srd/scripts/ 能否满足
    |
  [OK] SRD 搜索可直接覆盖 -> 在 code/ 中创建引用函数，内部调用 SRD 脚本
  [OK] SRD 可提供所需数据 -> 在 code/ 中创建数据提取函数，寄生 SRD 搜索
...
当判定为 SRD 可覆盖时，在 code/ 中创建轻量包装函数，而非复制 SRD 数据：
...
第一步：搜索 srd/scripts/ 能否满足
  ...
  [NO] SRD 无法覆盖 -> 走第二步
    |
第二步：搜索 code/CODE_LIBRARY.md
  [OK] 找到匹配函数 -> 直接调用
  [NO] 找不到 -> 由大模型生成代码
    |
函数化改造 -> 登记到 code/ -> 更新 CODE_LIBRARY.md
```

> **违反规则的后果**：若在 SRD 可覆盖的情况下仍自建数据（如手动硬编码法术列表到 code/ 中），属于 **重复建设**。
...
玩家提出新需求 → 生成函数 → 入库 `code/<category>/` → 更新 `CODE_LIBRARY.md` → 告知玩家已录入。

### 7.4 模板-代码同步约束
（原 DM_RULES.md 规则18.6）

`DM_TEMPLATES.md` 格式与 `code/` 函数输出必须一致。模板改→代码改，反之亦然。参见开发规则5。
