...
将 `dnd-dm-skill-x.x.x.zip` 解压到空目录。解压后结构如下：

```
your-workspace/
├── SKILL.md / _meta.json          ← Skill 元数据
├── CHANGELOG.md                  ← 版本历史
├── references/                   ← 14 个核心引用文件（供 LLM 读取）
...
│   └── party-sheet.html          ← 角色卡网页（双击可看）
├── code/                         ← [已弃用] 旧代码模板库（兼容保留）
├── items/                        ← 物品模板（至恶护符、炼狱头巾、Blackrazor）
└── __pycache__/                  ← 自动生成
```
...
```bash
pip install dnd-engine
dnd-engine init
```

自动创建：
- `saves/` — 存档目录
- `rules/` — 规则书目录（内含 README.txt 引导）
- `modules/` — 模组目录（内含 README.txt 引导）
- `live_party.json` — 角色状态模板
...
将以下文件放入对应目录：
- `rules/` — 2024版 D&D 三宝书（《玩家手册（2024）》.md、《城主指南（2024）》.md、《怪物图鉴（2025）》.md）
- `modules/` — 想玩的 D&D 模组文件（如《博德之门：坠入阿弗纳斯》.md 等）

### ▸ 步骤 4：启动 API 服务

```bash
dnd-engine server --port auto
# 或 python dnd-engine/api/server.py
```

`--port auto` 自动检测端口（优先 8080，被占用则从 8010-8020 选可用端口），端口号写入 `_api_port.txt`。

### ▸ 步骤 5：校验环境

在对话中回复 `/verify`，Agent 自动调用 `GET /api/system/verify` 校验所有依赖：
...
| 命令 | 功能 |
|:-----|:------|
| `dnd-engine init` | 初始化工作目录（创建 saves/ rules/ modules/ live_party.json） |
...
**版本 1.1.4 · 2026-06-08 · SKILL.md标准化(AgentSkills格式+metadata/os/install), 规则6层重排+layer标记修复, DM_RULES_INDEX.md移除, 安装引导流程完整化(verify+init+CLI) · 三层架构（dnd-engine + dnd-api + dnd-dm Skill）· 基于2024版D&D 5e规则 · 90项测试全部通过 · 支持 Windows / macOS / Linux*
