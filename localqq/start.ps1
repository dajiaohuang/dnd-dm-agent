...
Write-Host "=== NapCat Local QQ ===" -ForegroundColor Cyan

# ---------- QR helper ----------
function Show-QR {
    param([int]$TimeoutSec = 30)
    $cacheDir = Join-Path $Base "NapCat.44498.Shell\versions\9.9.26-44498\resources\app\napcat\cache"

    # Record existing timestamps to detect new files
    $before = @{}
    Get-ChildItem $cacheDir -Filter "*.png" -ErrorAction SilentlyContinue |
        ForEach-Object { $before[$_.Name] = $_.LastWriteTime }

    # Check if a QR already exists (generated before we started watching)
    $existing = Get-ChildItem $cacheDir -Filter "*.png" -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Where-Object { $_.Length -gt 1024 } |
        Select-Object -First 1
    if ($existing) {
        Write-Host "[OK] QR found: $($existing.Name) ($($existing.Length) bytes)" -ForegroundColor Green
    ...
    for ($i = 0; $i -lt $TimeoutSec; $i++) {
        ...
        if ($qr) {
            Write-Host "[OK] QR: $($qr.Name) ($($qr.Length) bytes)" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "[!] No QR appeared. Check QQ window or open cache folder:" -ForegroundColor DarkYellow
...
if (-not $QQRunning) {
    Write-Host "[..] Starting local QQ..." -ForegroundColor Yellow
...
# Wait for WebSocket
Write-Host "[..] Waiting for NapCat WebSocket..." -ForegroundColor Yellow
for ($i = 0; $i -lt 20; $i++) {
