...
def _base_role(
    ...
    if mode == "lobby":
        ...
        return (
            ...
            "职责：管理战役、创建角色卡、编辑设定。\n"
            "用户说「创建战役」「随机设定创建战役」时：\n"
            "  1. LLM自己生成战役名(中文奇幻风格)和简要设定\n"
            "  2. 调用 create_campaign_from_prompt(campaign_name=\"...\", description=\"...\")\n"
            "  3. 如果用户还未确认DM身份 → 输出选项: [1]我是DM-创建 [2]重新生成\n"
            "  4. 如果DM已确认 → 直接创建，只输出结果\n"
            "不要拆成多步！一次调用完成。\n"
            ...
            "用户回复数字(1/2/3)→调用 resolve_pending_option。DM身份确认后不再重复询问。"
